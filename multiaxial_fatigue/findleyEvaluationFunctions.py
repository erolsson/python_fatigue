...
def evaluate_findley(combined_stress, a_cp, worker_run_out_time, chunk_size, num_workers=multiprocessing.cpu_count(), 
    ...
    if True:
        ...
        # Read pickle dump file
        print " unPickle Findly stresses from file,",
        pickle_handle = open('Findley2.pkl', 'rb')    
        rows, cols = pickle.load(pickle_handle)
        fatigue_results = np.empty([rows, cols], dtype=float)
        fatigue_results[:, :] = pickle.load(pickle_handle)
        pickle_handle.close()
        print "Done!"
    if not w_pool:
        worker_pool.close()
