...
Roller = namedtuple('Roller', ['name', 'radius', 'b', 'h'])
Specimen = namedtuple('Specimen', ['L', 'R', 'q', 't', 'h', 'h1', 'CD', 'delta', 'name'])


def create_roller(model_db, radius, name, b, h):
    p1 = (0.0, radius)
    p2 = (-radius*sin(0.9*pi/2), radius*(1-cos(0.9*pi/2)))
    p3 = (radius*sin(0.9*pi/2), radius*(1-cos(0.9*pi/2)))

    # Create sketch 
    indenter_sketch = model_db.ConstrainedSketch(name=name + '_sketch', sheetSize=800.0)
    ...
    roller = model_db.Part(name=name, dimensionality=THREE_D,
