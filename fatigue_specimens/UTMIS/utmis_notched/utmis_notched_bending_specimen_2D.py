...
class NotchedBendingSpecimenClass:
    ...
    def make_part(self, part_name='fatiguePart'):
        ...
        self.fatigue_part.PartitionFaceByDatumPlane(datumPlane=self.fatigue_part.datum[datum_plane_vertical2.id],
                                                    faces=self.fatigue_part.faces)

        datum_plane_vertical3 = self.fatigue_part.DatumPlaneByPrincipalPlane(principalPlane=YZPLANE, offset=self.x3)
        self.fatigue_part.PartitionFaceByDatumPlane(datumPlane=self.fatigue_part.datum[datum_plane_vertical3.id],
                                                    faces=self.fatigue_part.faces)
    ...
    def mesh(self, part=None, flip=False, analysis_type='ThermalDiffusion'):
        ...
        sweep_faces = part.faces.findAt(((1e-6, 1e-6, 0),),
                                        ((1e-6, self.notch_height/2 - self.case_mesh_thickness/2, 0),),
                                        ((self.x + 1e-6, 1e-6, 0),))

        part.setMeshControls(regions=sweep_faces, elemShape=QUAD, technique=SWEEP)
        ...
        f1 = part.edges.findAt((1.001*self.x3, self.height/2, 0.))

        #              Pick connected edges by angle
        exposed_edges = f1.getEdgesByEdgeAngle(89)
        #              Get elements and nodes connected to selected edges
        exposed_nodes = nodes_on(exposed_edges)
        exposed_elements = elements_on(exposed_edges)
        #              Create sets on part
        ...
        # Symmetry planes
        f = part.edges.findAt((0., self.notch_height/4, 0))
        x_sym_edges = f.getEdgesByEdgeAngle(0)
        x_sym_nodes = nodes_on(x_sym_edges)
        x_sym_elements = elements_on(x_sym_edges)
        part.Set(nodes=x_sym_nodes, name='XSym_Nodes')
        part.Set(elements=x_sym_elements, name='XSym_Elements')

        f = part.edges.findAt((1.001*self.x3, 0, 0))
        y_sym_edges = f.getEdgesByEdgeAngle(1)
        y_sym_nodes = nodes_on(y_sym_edges)
        y_sym_elements = elements_on(y_sym_edges)
    ...
    def change_analysis_type(self, analysis_type='Mechanical'):
        ...
        # Assign element type       
        self.myAssembly.setElementType(regions=(self.fatigue_part.faces,), elemTypes=(elem_type1, elem_type2))
...
if __name__ == "__main__":
    ...
    m.write_file('utmis_notched_2d.inc')
